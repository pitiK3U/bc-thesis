...
The distance sensor (\acrshort{lidar}) is placed in the middle of each dock and is described in detail in its own chapter (chapter \ref{sec:lidar}). The \acrshort{lidar} measures the distance from the dock to an obstacle or a surface, or to help align docks when connecting. The \acrshort{lidar} can scan the environment, which can be useful when the \gls{rofibot} needs to walk up stairs or needs to know about its surroundings.
...
The architecture of the \gls{universal-module} is shown in figure \ref{fig:uni-mod-arch}.

\begin{figure}[ht]
    \includegraphics{ assets/universal-module-arch.pdf }
    \caption{Architecture of the \gls{universal-module} from \TODO{\cite{Mrazek2019thesis} page 35}}
...
The actual changes to the prototype implementation are described later in chapter \ref{cht:rofi-hal}.
...
\chapter[ RoFI Hardware Abstraction Layer ]{ \acrshort{rofi} \acrlong{hal} } \label{cht:rofi-hal}
