...
\gls{universal-module} \glsdesc{universal-module} The \gls{universal-module} is not designed as all-in-one module, but rather as minimal needed module that can be used in nearly all systems. Thus, \gls{universal-module} uses only two sensors: an inertial measurement unit (IMU) and a distance sensor (\acrshort{lidar}). If specific sensors are needed, it is expected to use them as own modules.
...
Each module has a single \emph{control unit} that serves as a centralized control over modules actions and all the module components. The motors are controlled trough UART protocol and dock control is over SPI protocol. The control unit also controls charging and power management.
