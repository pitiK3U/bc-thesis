...
\thesissetup{
    ...
    abstract    = {%
      % Background
      The \acrshort{rofi} platform is an open-software and open-hardware platform for modular
      robots. The \acrshort{rofi} platform uses the \acrshort{rofi} Communication Mechanism,
      \acrshort{roficom} for short, for connecting and communicating between two \acrshort{rofi}
      modules.
      
      % Objective
      This thesis presents an implementation of firmware for RoFiCoM. It is built on top an existing
      prototype implementation and transforms it into a production-ready state. We primarily
      introduce a board support package to support various hardware revisions and implement missing
      functionality, such as utilizing all on-board sensors and reworking motor control.
      Additionally, we enhance the communication protocol with support for these features and extend
      implementation for the Universal module's RoFICoM driver to support these changes.
    },
    thanks      = {%
        I would like to thank the ParaDiSe laboratory for a welcoming place to work,
        Jan Mrázek for introduction to the \acrshort{rofi} platform, and everybody who supported me
        during the writing of this thesis.
    },
    bib         = used-citations.bib,
...
% Introduce topic
The \acrshort{rofi} platform is an open software and open-hardware platform for modular robots. The
\acrshort{rofi} platform utilizes the \acrshort{rofi} Communication Mechanism, \acrshort{roficom}
for short, for connecting and communicating between two \acrshort{rofi} modules.

This thesis improves the currently implemented firmware for the connector (\acrshort{roficom}) of
the \acrshort{rofi} platform.
...
The docks used in the \gls{universal-module} are called \acrshortpl{roficom}. The \acrshort{roficom}
...
for establishing strong mechanical connections between two \acrshortpl{roficom}.  The clip component
...
\path{RoFI/hardwareModules/RoFICoM/hardware/control_board/}.
...
\path{RoFI/hardwareModules/RoFICoM/software/control_board/}.
...
\path{RoFI/softwareComponents/stm32cxx/port/}. The \verb|port| files are in the format
\verb|{driver}.port.hpp|. For example, the port file for \acrshort{i2c} for the microcontroller of
\acrshort{roficom} is in \path{.../stm32cxx/stm32g0xx/i2c.port.hpp}.
...
\path{RoFI/softwareComponents/atoms/include/atoms/result.hpp}.
...
\path{RoFI/softwareComponents/stm32cxx/src/drivers/}.
...
\path{RoFI/softwareComponents/rofiHalInc/include/rofi_hal.hpp}.
