...
\clearpage
NOTE FOR VB000Eng: Even though this document contains the full current version of my thesis, for the review, I am submitting only the chapter RoFI \ref{sec:rofi}, which should be around the limit of 5 ``normostrana'' of text.

\TODO{Again text in this style indicates that the marked text needs reworking, there is something missing like a reference or information that may be changed or inaccurate.}
\printnoidxglossary[title={Abbreviations}, type=\acronymtype]
\printnoidxglossary[title={Glossary}]

\chapter{Introduction}

\chapter[ RoFI ]{ \acrshort{rofi} } \label{sec:rofi}
\TODOLIST{
\begin{itemize}
...
\acrlong{roficom}, \acrshort{roficom} for short, is an open-hardware connection system designed for the \acrshort{rofi} platform. \acrshort{roficom} focuses on reusability in order to make it easily integrable into any module for which \acrshort{roficom} provides
...
The \acrshort{rofi} repository is an enormous monolithic project and contains many \acrshort{rofi} related sub-projects. For this thesis, only several sub-projects are essential. Those sub-projects are the \acrshort{roficom}'s firmware, stm32 drivers and \acrshort{rofi} HAL.
...
The \acrshort{rofi} \acrshort{hal} is divided into \emph{interface} and \emph{implementation}. \TODO{Text about the interface, that chagnes in the interface are costly.} However, due to the addition of \acrshort{lidar}, \TODO{the interface needs an update to reflect this addition}.
